...
function handleShuffleGame(socket, io) {
  return (data, callback) => {
    try {
      ...
      // إعادة ترتيب الحروف
      rooms[roomId].gameState.hexGrid = shuffleHexGrid(
        rooms[roomId].gameState.hexGrid,
        config.game.arabicLetters
      );
      
      console.log(`تم إعادة ترتيب الحروف في الغرفة ${rooms[roomId].name}`);
      
      // إعلام جميع اللاعبين في الغرفة بإعادة ترتيب الحروف
      io.to(roomId).emit('game:state', rooms[roomId].gameState);
      
      callback({ 
        success: true, 
        hexGrid: rooms[roomId].gameState.hexGrid
      });
    } catch (error) {
      console.error('خطأ في إعادة ترتيب الحروف:', error);
      callback({ error: 'حدث خطأ أثناء إعادة ترتيب الحروف' });
    }
  };
}

/**
 * معالج تغيير ألوان الفرق
 */
function handleChangeColors(socket, io) {
  return (data, callback) => {
    try {
      ...
      // التحقق من أن المرسل هو المضيف
      if (rooms[roomId].hostId !== socket.id) {
        return callback({ error: 'فقط المضيف يمكنه تغيير الألوان' });
      }
      ...
      // تحديث ألوان الخلايا في الشبكة السداسية
      rooms[roomId].gameState.hexGrid = updateTeamColors(
        rooms[roomId].gameState.hexGrid,
        rooms[roomId].teamColors
      );
      
      console.log(`تم تغيير ألوان الفرق في الغرفة ${rooms[roomId].name}`);
      
      // إعلام جميع اللاعبين في الغرفة بتغيير الألوان
      io.to(roomId).emit('room:update', rooms[roomId]);
      io.to(roomId).emit('game:state', rooms[roomId].gameState);
      
      callback({ 
        success: true, 
        teamColors: rooms[roomId].teamColors,
        hexGrid: rooms[roomId].gameState.hexGrid
      });
    } catch (error) {
      console.error('خطأ في تغيير ألوان الفرق:', error);
      callback({ error: 'حدث خطأ أثناء تغيير ألوان الفرق' });
    }
  };
}
...
function handleAnnounceWin(socket, io) {
  return (data, callback) => {
    try {
      ...
      rooms[roomId].gameState.winner = team;
      rooms[roomId].gameState.endedAt = new Date().toISOString();
      
      console.log(`تم إعلان فوز الفريق ${team} في الغرفة ${rooms[roomId].name}`);
      
      // إعلام جميع اللاعبين في الغرفة بإعلان الفوز
...
function handleDisconnect(socket, io) {
  return () => {
    try {
      ...
      // البحث عن اللاعب بواسطة معرف Socket
      const playerId = Object.keys(players).find(id => players[id].socketId === socket.id);
      
      if (playerId && players[playerId]) {
        const roomId = players[playerId].room;
        
        // إذا كان اللاعب في غرفة، قم بإزالته منها
        if (roomId && rooms[roomId]) {
          // استخدام معالج مغادرة الغرفة
          handleLeaveRoom(socket, io)({ roomId, playerId }, () => {});
        }
        
        // تحديث حالة اللاعب
        players[playerId].status = 'offline';
        players[playerId].disconnectedAt = new Date().toISOString();
      }
    } catch (error) {
